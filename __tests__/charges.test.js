...
describe('Charges Endpoints', () => {
  ...
  test('create a charge with a card token', () => {
    ...
    return api.createCard(testCard)
      .then(cardResponse =>
        actual({
          ...testCharge,
  ...
  test('create charge with a customers token', () => {
    ...
    const expectedChargeWithCustomer = { ...expectedCharge, card: { ...card, customer_token: expect.any(String), primary: true } }
    ...
    return api.createCustomer(testCustomer)
      .then(customerResponse => 
        ...
        }).then(chargeResponse => expect(chargeResponse).toEqual(expectedChargeWithCustomer))
      )
  })
})
