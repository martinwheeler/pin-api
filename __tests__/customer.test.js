...
describe.skip("Customer Endpoints", () => {
  ...
  test("fetching a created customer", () => {
    expect.assertions(2);
    return api
      .fetchCustomer(testCustomerToken)
      .then(response => {
  ...
  test("updating a customer returns changed value", () => {
    expect.assertions(1);
    return api
      .updateCustomer(testCustomerToken, {
        card: {
          ...testCard,
          name: "Mrs Robot"
        }
  ...
  test("can create additional cards for a customer", () => {
    const actual = api.createCustomerCard(
      { ...testCard, name: "Fake Card Name", number: 4200000000000000 },
      testCustomerToken
    );
    const expected = {
      address_city: "Brisbane",
  ...
  test("delete a customer returns 204", () => {
    expect.assertions(1);
    return api.deleteCustomer(testCustomerToken).then(response => {
